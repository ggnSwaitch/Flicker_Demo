...
typealias ImageDownloadHandler = (_ image: UIImage?, _ url: URL, _ indexPath: IndexPath?,_ title: String, _ error: Error?) -> Void
...
final class ImageDownloadManager {
    ...
    func downloadImage(_ flickrPhoto: FlickrPhoto, indexPath: IndexPath?, title: String, size: String = "m", handler: @escaping ImageDownloadHandler) {
        ...
        if let cachedImage = imageCache.object(forKey: url.absoluteString as NSString) {
            ...
            self.completionHandler?(cachedImage, url, indexPath, title, nil)
        } else {
            ...
            if let operations = (imageDownloadQueue.operations as? [DownloadOperation])?.filter({$0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true }), let operation = operations.first {
                ...
                let operation = DownloadOperation(url: url, indexPath: indexPath, title: title)
                if indexPath == nil {
                    operation.queuePriority = .high
                }
                
                operation.downloadHandler = { (image, url, indexPath,title, error) in
                    if let newImage = image {
                        self.imageCache.setObject(newImage, forKey: url.absoluteString as NSString)
                    }
                    self.completionHandler?(image, url, indexPath,title, error)
                }
